...
// styles
import styles from './GalleryCategories.module.sass';

const GalleryCategories = () => {
	const [categoriesList, setCategoriesList] = useState(['category']);
	const [isLoading, setIsLoading] = useState([]);
	const { setGalleryName, setGalleryImages } = useGalleryContext();
	...
	useEffect(() => {
		setCategoriesList(data?.galleryCategoriesPlural);
	}, [data]);

	/* ----------------------------- DISPLAY GALLERY ---------------------------- */

	const displayGallery = (index) => {
		setGalleryName(categoriesList[index]?.categoryName);
		setGalleryImages(categoriesList[index]?.categoryImages);
	};
	...
	const renderCategories = () => {
		return categoriesList?.map((category, index) => {
			return (
				<div
					onClick={() => displayGallery(index)}
					key={'category' + index}
					className={styles.category}
					style={{
						backgroundImage: `url(${category?.mainPhoto?.url})`,
					}}>
					{loading ? (
